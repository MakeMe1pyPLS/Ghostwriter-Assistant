...
const STEPS = [
  { id: 'sector', title: 'Business Context', subtitle: 'What type of business do you run?' },
...
function OptionCard({ selected, onClick, icon: Icon, label, description, recommended }: {
  selected: boolean; onClick: () => void; icon: any; label: string; description?: string; recommended?: boolean;
}) {
  return (
    <button
      onClick={onClick}
...
export function GenerateWizard({ onComplete, onCancel, loading }: GenerateWizardProps) {
  ...
  const [answers, setAnswers] = useState<WizardAnswers>({
    sector: '', goal: '', tool: '', style: '', kpiPriorities: [], dataContext: '', density: '', aiHelpLevel: '',
  });
  ...
  const canProceed = () => {
    const s = STEPS[step].id as keyof WizardAnswers;
    if (s === 'kpiPriorities') return answers.kpiPriorities.length > 0;
    return !!answers[s];
  };
      ...
      case 0:
        return (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <OptionCard icon={ShoppingCart} label="E-commerce" description="Online retail, DTC, marketplace operations" selected={answers.sector === 'ecommerce'} onClick={() => updateAnswer('sector', 'ecommerce')} />
            <OptionCard icon={Truck} label="Logistics" description="Shipping, warehousing, transportation" selected={answers.sector === 'logistics'} onClick={() => updateAnswer('sector', 'logistics')} />
            <OptionCard icon={Building2} label="Manufacturing" description="Production, quality, assembly operations" selected={answers.sector === 'manufacturing'} onClick={() => updateAnswer('sector', 'manufacturing')} />
            <OptionCard icon={Layers} label="Unified Supply Chain" description="End-to-end multi-sector visibility" selected={answers.sector === 'unified'} onClick={() => updateAnswer('sector', 'unified')} recommended />
            <OptionCard icon={Puzzle} label="Custom" description="Mix KPIs from any category" selected={answers.sector === 'custom'} onClick={() => updateAnswer('sector', 'custom')} />
          </div>
        );
      case 1:
        return (
